...
def map_columns(df):
    ...
    # df['obtained_id'] = df['org_string'].apply(get_org_id)
    return df

# def api_choice(org_string):
#     if 

def get_charity_reg(org_string):
    pass
...
def get_org_id(org_string):
    ...
    org_id = comp_house_dict['items'][0]['company_number']

    # if org_type is charity, use charity register:

    return org_id

# get_comp_house_info("001 INSPIRATION  LIMITED")
