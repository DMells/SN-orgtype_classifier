...
def get_org_id(df, session=None):
    """
    Lookup companies house company name and return the company number

    :param df: pandas dataframe containing the organisation name
    :param session: A requests Session() instance
    :return: Amended dataframe containing additional company information
    """
    if not session:
    ...
    for chunk in np.array_split(org_strings, math.ceil(len(org_strings)/5), axis=0):
        ...
        for word in chunk:
            pdb.set_trace()
            authString = base64.b64encode('{companieshouse_key}:'.format(**{
            'companieshouse_key': b'COMPANIES_HOUSE_API_KEY'}))
            headers = {'Authorization': 'Basic {0}'.format(authString)}
            url = "https://api.companieshouse.gov.uk/search/companiesq={0}".format(word)
            status = 429
            while status == 429:
                ...
                if response.status_code == 200:
                    ...
                    logger.debug("Error requesting CH data: %s %s", response.status_code, response.reason)
                elif status == 429:
                    logger.debug("Error requesting CH data: %s %s", response.status_code, response.reason)
                    ...
                    logger.error("Error requesting CH data: %s %s", response.status_code, response.reason)
